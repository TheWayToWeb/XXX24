...
import TopMenuView from "./TopMenuView.jsx"; // Импортируем презентационный компонент

const TopMenuSmart = () => {
    const [active, setActive] = useState(null); // Начальное состояние активного элемента
    ...
    const handleClickLink = (item) => {
        ...
        /*handleShowModal(item); */// Показываем модальное окно в зависимости от свойства demonstrated
    };

    /*const handleShowModal = (item) => {
        setShow(item.demonstrated);
    };*/
    ...
    return (
        <>
            <TopMenuView
                initItems={data}
                active={active}
                handleClickLink={handleClickLink} // Передаем функцию обработки клика
            />
            {/*{show && <TopModalWindowView />}*/} {/* Отображаем модальное окно, если show истинно */}
        </>
    );
};

export default TopMenuSmart;